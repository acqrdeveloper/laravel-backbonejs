define([
    ...
    function ($, Backbone, toastr, UserRouter, UserClass) {
        ...
        //todo: muestra las url directas del route del backend.
        Backbone.history.start({
            pushState: true,
            silent: false,
            root: rootPath
        });
        //todo: muestra las url directas del route del backend y frontend #.
        // Backbone.history.start();
        ...
        $(document).on('click', 'a:not([data-bypass])', function (e) {
            ...
            if (href.length && href.substr(0, 1) != '#') {
                ...
                UserRouter.navigate(getRelativeUrl(href, rootUrl), {trigger: true});
            }
        });
        ...
        $(document).on('submit', 'form:not([data-bypass])', function (evt) {
            ...
            if (href.length && href.substr(0, 1) != '#') {
                ...
                $.ajax({
                    ...
                    UserRouter.navigate(getRelativeUrl(xhr.responseURL, rootUrl));
                }).always(function () {
